...
// Vec2.normalize = function (x, y,lenght) {
//     var normal = 0;
//     var lenght = sqrt((x*x) +(y*y));
//
// };
...
CurveHermite.prototype.a = function(){
    a = new Vec2();
    a.x = 2*this.p_a.x-2*this.p_b.x+1*this.p_c.x+1*this.p_d.x;
    a.y = 2*this.p_a.y-2*this.p_b.y+1*this.p_c.y+1*this.p_d.y;
    return a;
}

CurveHermite.prototype.b = function(){
    b = new Vec2();
    b.x = -3*this.p_a.x+3*this.p_b.x-2*this.p_c.x-1*this.p_d.x;
    b.y = -3*this.p_a.y+3*this.p_b.y-2*this.p_c.y-1*this.p_d.y;
    return b;
}

CurveHermite.prototype.c = function(){
    c = new Vec2();
    c.x = 0*this.p_a.x+0*this.p_b.x+1*this.p_c.x+0*this.p_d.x;
    c.y = 0*this.p_a.y+0*this.p_b.y+1*this.p_c.y+0*this.p_d.y;
    return c;
}
...
function ArcLength(type){
    this.u1=0;
    this.u2=0;
    this.length=0;
    this.table=[]; //lista de Table_Entry
    if(type == "Bezier" || type == "bezier"){
    this.curve= new CurveBezier();
  }else{ //if(type == "Hermite" || type == "hermite"){
    this.curve= new CurveHermite();
  }
  //}else if (type == "BSpline"){
    //this.curve = new CurveBSpline();
  //}
...
// function CurveBSpline(p_a,p_b,p_c,p_d){
//     this.p_a = p_a;
//     this.p_b = p_b;
//     this.p_c = p_c;
//     this.p_d = p_d;
...
// }
//
// CurveBSpline.prototype.a = function(){
//     a = new Vec2();
//     a.x = ((-1)*this.p_a.x+3*this.p_b.x-3*this.p_c.x+1*this.p_d.x);
//     a.y = ((-1)*this.p_a.y+3*this.p_b.y-3*this.p_c.y+1*this.p_d.y);
//     return a;
// }
//
// CurveBSpline.prototype.b = function(){
//     b = new Vec2();
//     b.x = (3*this.p_a.x-6*this.p_b.x+0*this.p_c.x+4*this.p_d.x);
//     b.y = (3*this.p_a.y-6*this.p_b.y+0*this.p_c.y+4*this.p_d.y);
//     return b;
// }
//
// CurveBSpline.prototype.c = function(){
//     c = new Vec2();
//     c.x = ((-3)*this.p_a.x+3*this.p_b.x+3*this.p_c.x+1*this.p_d.x);
//     c.y = ((-3)*this.p_a.y+3*this.p_b.y+3*this.p_c.y+1*this.p_d.y);
//     return c;
// }
// CurveBSpline.prototype.evaluate = function(t) //parametro 0<=t<=1
// {
//
//     var b0,b1,b2,b3;
//     b0 = ((-1)*(t*t*t) + 3*t*t -3*t + 1);
//     b1 = ((+3)*t*t*t -6*t*t + 4);
//     b2 = ((-3)*t*t*t +3*t*t + 3*t + 1);
//     b3 = (t*t*t);
//     p = new Vec2();
//     p.x = (this.p_a.x*b0 + this.p_b.x*b1 + this.p_c.x*b2 + this.p_d.x*b3);
//     p.x = p.x/6;
